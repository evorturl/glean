...
if [[ -z "${GLEAN_ALLOWED_USER_EMAILS:-}" && -z "${GLEAN_ALLOWED_USER_EMAIL:-}" && -z "${GLEAN_CLIENT_ACT_AS:-}" ]]; then
  echo "Missing GLEAN_ALLOWED_USER_EMAILS, GLEAN_ALLOWED_USER_EMAIL, or GLEAN_CLIENT_ACT_AS in ${VARIABLES_ENV_FILE}."
  exit 1
fi
